...
const gleispunkte = d3.csvParse(fs.readFileSync("fixtures/gleispunkte.csv", "utf-8"), row => {
  for (const k in row) {
    switch (k) {
      case "ERSETZT":
      case "ERSTELLT":
...
const gleiskanten = d3.csvParse(fs.readFileSync("fixtures/gleiskante.csv", "utf-8"), row => {
  for (const k in row) {
    switch (k) {
      case "ERSETZT":
      case "ERSTELLT":
...
const gleisknoten = d3.csvParse(fs.readFileSync("fixtures/gleisknoten.csv", "utf-8"), row => {
  for (const k in row) {
    switch (k) {
      case "ERSETZT":
      case "ERSTELLT":
